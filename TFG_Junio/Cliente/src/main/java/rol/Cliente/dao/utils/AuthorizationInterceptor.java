...
public class AuthorizationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (ca.getPass() != null || ca.getCorreo() != null) {
            if (ca.getToken() == null) {
                request = original.newBuilder()
                        .header(ConstantesParametros.AUTHORIZATION_HEADER, Credentials.basic(ca.getCorreo(), ca.getPass())).build();
            } else {
            ...
            if (response.header(ConstantesParametros.EXPIRES_HEADER) != null) {
                ...
                for (int i = 0; i < 5; i++) {
                    if (ca.getToken() == null) {
                        request = original.newBuilder()
                                .header(ConstantesParametros.AUTHORIZATION_HEADER,
                                        Credentials.basic(ca.getCorreo(), ca.getPass())).build();
